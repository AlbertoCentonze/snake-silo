...
interface Silo:
    def borrow_possible(asset: address, borrower: address) -> bool: nonpayable
...
MINIMUM_SHARE_AMOUNT: public(constant(uint256)) = 100000
...
@external
@payable
def __init__(name_: String[25], symbol_: String[5], silo_: address, asset_: address):
    """
    ...
    silo = Silo(silo_)
    asset = asset_
    # SILO: decimals are inherited from the asset
    self.decimals = ERC20Detailed(asset).decimals()
    name = name_
    symbol = symbol_

    self._transfer_ownership(silo_)
    self.is_minter[msg.sender] = True
    log RoleMinterChanged(msg.sender, True)

    # SILO: removed unnecessary initial supply logic since it is not needed for silo

    # SILO: removed unnecessary EIP712 logic since it is not needed for silo
    _NAME = "snake-silo"
    _VERSION = "snake-silo"
    _HASHED_NAME = keccak256("snake-silo")
    _HASHED_VERSION = keccak256("snake-silo")
...
@internal 
def _notify_about_transfer(from_: address, to: address, amount: uint256): 
...
def _is_transfer(sender: address, recipient: address) -> (bool):
        return sender != empty(address) and recipient != empty(address)
...
def _before_token_transfer(owner: address, to: address, amount: uint256):
    ...
    @param amount The 32-byte token amount to be transferred.
    """
    # SILO: implemented before token transfer logic inside snekmate implementation
    if not self._is_transfer(owner, to):
        return
    
    if not silo.borrow_possible(asset, to):
        raise "Share Transfer Not Allowed"

    # SILO: skipping ERC20R logic for now
...
def _after_token_transfer(owner: address, to: address, amount: uint256):
    ...
    @param amount The 32-byte token amount that has
           been transferred.
    """
    # SILO: implemented after token transfer logic inside snekmate implementation
    if self._is_transfer(owner, to):
            return
    ...
    if self._is_transfer(owner, to) and not silo.is_solvent(to):
        raise "Recipient Not Solvent After Transfer"
